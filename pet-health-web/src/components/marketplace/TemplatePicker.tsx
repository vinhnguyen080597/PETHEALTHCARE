...
const templatePreviews: Record<
  TemplateId,
  { bg: string; text: string; accent: string; preview: string }
> = {
  T1: {
    bg: "bg-gradient-to-r from-[#1E6FE8] to-[#2563EB]",
    text: "text-white",
    accent: "#1E6FE8",
...
export function TemplatePicker({
  ...
  const apply = async () => {
    ...
    setError("");
    try {
      const res = await fetch("/api/breeder/template", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          template: selected,
          metadata: { ...initialMetadata, template: selected },
        }),
      });
      ...
      <div className="flex items-start justify-between mb-2">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">
            Chọn giao diện trang trại
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Chọn 1 trong 5 template. Nội dung hồ sơ giữ nguyên khi đổi.
          </p>
        </div>
      </div>

      {error && (
        <p className="mt-4 text-sm text-red-600 bg-red-50 border border-red-100 rounded-xl px-4 py-2">
          {error}
        </p>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 mt-8">
          ...
          {tids.map((tid) => {
            ...
            return (
              <button
                ...
                className={`text-left rounded-xl border-2 overflow-hidden transition-all focus:outline-none focus:ring-2 focus:ring-[#1E6FE8]/40 ${
                  isSelected
                    ? "border-[#1E6FE8] shadow-md shadow-blue-100"
                    : "border-transparent hover:border-slate-200"
                }`}
              >
                <div
                  className={`h-24 ${preview.bg} flex items-center justify-center p-4 relative`}
                >
                  <div className="w-full">
                    <div
                      className={`text-xs font-bold truncate mb-1 ${preview.text}`}
                    >
                      Cattery Miu House
                    </div>
                ...
                <div className="p-3 bg-white">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {isCurrent && (
                      <span className="text-[10px] font-medium text-[#1E6FE8] bg-blue-50 px-1.5 py-0.5 rounded-full">
                        Hiện tại
                      </span>
                    )}
                  </div>
        ...
        <div className="lg:col-span-2 sticky top-24 self-start">
          <div className="bg-white rounded-xl border border-slate-100 overflow-hidden shadow-sm">
            <div
              className={`${templatePreviews[selected].bg} p-8 flex flex-col items-center justify-center min-h-48 text-center`}
            >
              <div
                className={`text-lg font-bold mb-1 ${templatePreviews[selected].text}`}
              >
                {selected} — {templateMeta[selected].nameVI}
              </div>
              <div
                className={`text-sm opacity-70 ${templatePreviews[selected].text}`}
              >
                {templateMeta[selected].nameEN}
              </div>
            </div>
            <div className="p-5">
              <p className="text-sm text-slate-700 leading-relaxed mb-4">
                {templateMeta[selected].description}
              </p>
              <button
                type="button"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm">
            <h2 className="font-bold text-slate-900 mb-2">
              Áp dụng template {selected}?
            </h2>
            <p className="text-sm text-slate-500 mb-6 leading-relaxed">
              Giao diện trang trại sẽ đổi sang{" "}
              <strong>{templateMeta[selected].nameVI}</strong>. Nội dung hồ sơ
              giữ nguyên.
            </p>
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={saving}
                onClick={apply}
                className="flex-1 py-2.5 bg-[#1E6FE8] text-white text-sm font-semibold rounded-full hover:bg-[#1D4ED8] disabled:opacity-50"
              >
