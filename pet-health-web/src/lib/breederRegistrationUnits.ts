...
export function isRegistrationUnitId(value: string): value is RegistrationUnitId {
  return Object.values(REGISTRATION_UNITS_BY_SPECIES)
    .flat()
    .includes(value as RegistrationUnitId);
}
...
export function registrationUnitLabelKey(
  unit: string,
): `breederForm.registrationUnits.${RegistrationUnitId}` | null {
  if (!unit || unit === REGISTRATION_UNIT_OTHER) return null;
  if (isRegistrationUnitId(unit)) {
    return `breederForm.registrationUnits.${unit}`;
  }
  return null;
}
...
export function displayRegistrationUnit(
  ...
  const key = registrationUnitLabelKey(unit);
