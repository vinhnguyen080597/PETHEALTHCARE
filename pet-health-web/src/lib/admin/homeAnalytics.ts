...
export type BreedSlice = { name: string; value: number };
...
/** Species mix across registered farms (primary_species). */
...
function topSlices(counts: Map<string, number>, limit: number): BreedSlice[] {
  ...
  const rest = sorted.slice(limit - 1).reduce((sum, [, n]) => sum + n, 0);
  return [...top.map(([name, value]) => ({ name, value })), { name: "Other", value: rest }];
}
