...
test('sen abandon after handoff republishes listing and stamps escrow forfeit', async () => {
  ...
  const created = await createMyWarrantyPolicy(breederId, {
    ...samplePolicy,
    title: 'Policy Abandon',
  }, null);
  let post = await createPetFeedPost(breederId, {
    title: 'Cat Abandon',
    species: 'cat',
    status: 'draft',
    mediaUrls: ['https://cdn.example.com/abandon.jpg'],
    videoUrl: 'https://cdn.example.com/abandon.mp4',
    metadata: { warranty_policy_id: created.policy.id },
  }, null);
  post = await adminUpdatePetFeedPostStatus(post.id, 'published');
  await senThenBreederConfirmDeposit(breederId, senId, post.id);
  ...
  const abandoned = await abandonListingHandoffBySen(senId, post.id, null);
  assert.equal(abandoned.post.status, 'published');
  assert.equal(abandoned.escrow_forfeit_to_breeder, true);
  assert.equal(abandoned.notify_user_id, breederId);
  assert.equal(abandoned.post.metadata?.deal?.sen_user_id, null);
