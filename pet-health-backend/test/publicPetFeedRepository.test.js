...
test('published column + cancelled metadata is closed, not listed as for-sale', async () => {
  const userId = `public-cancelled-${Date.now()}`;
  await upsertMyBreederProfile(userId, {
    displayName: 'Cancelled Farm',
    bio: 'Indoor cats',
    location: 'TP.HCM',
    contact: { zalo: '0901111222' },
    primarySpecies: ['cat'],
  ...
  const created = await createPetFeedPost(userId, {
    ...
    dewormingStatus: 'Done',
    paperwork: ['Vaccine book'],
    mediaUrls: ['https://cdn.example/cancelled.jpg'],
    status: 'pending_review',
    metadata: {
      health_evidence_urls: ['https://cdn.example/vaccine-book.jpg'],
      cancelled: true,
