...
} from './breederProfileSubmissions.ts';
import { pickLangText, type TrustGuideHowToEarn, type TrustGuideLang } from './farmTrustGuide.ts';
import {
...
export function earnRowCtaLabelKey(cta: Exclude<EarnRowCta, 'none'>): EarnRowCtaLabelKey;
...
/** Map flat web-style key to nested i18n path used by react-i18next. */
export function earnRowCtaI18nPath(cta: EarnRowCta): string | null {
  if (cta === 'pending') return 'farm.trust.guide.earnPending';
  if (cta === 'rejected') return 'farm.trust.guide.earnRejected';
  if (cta === 'update') return 'farm.trust.guide.earnUpdate';
  return null;
...
export function earnRowCta(id: string, _done: boolean, queueStatus: EarnQueueStatus): EarnRowCta {
...
export function earnRowMediaKind(id: string, url: string | null | undefined): EarnRowMediaKind | null {
...
export function buildTrustGuideEarnRowStates(
  ...
  return rows.map((row) => {
    ...
    return {
      ...
      description: mediaUrl ? '' : earnRowDescription(row, ctx.lang, approvedValue, done),
      descriptionHref: mediaUrl || !(done && approvedValue) ? null : earnRowApprovedHref(approvedValue),
      mediaKind,
      mediaUrl,
    };
  });
...
export function earnModalView(state: { busy: boolean; submitted: boolean }): EarnModalView {
...
export function isUploadSubmissionType(type: string): boolean {
  return type === 'facility_video' || type === 'business_license';
}

export function isEarnSubmissionSocial(action: TrustGuideEarnAction): boolean {
  return action.kind === 'submission' && isSocialSubmissionType(action.submissionType);
}
