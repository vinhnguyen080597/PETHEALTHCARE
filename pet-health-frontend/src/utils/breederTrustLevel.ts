export type TrustLevelId = 'L2' | 'L3' | 'L4' | 'L5';
...
export function trustLevelFromScore(score: number): TrustLevelSummary {
  ...
  // Below 30 only happens before approval — floor to the entry title.
  return { level: 'L2', labelKey: 'breederTrustLevel.L2' };
}

export function levelColor(level: string): TrustLevelColors {
  if (level === 'L5') return { bg: '#D1FAE5', text: '#065F46', dot: '#059669' };
  if (level === 'L4') return { bg: '#D1FAE5', text: '#065F46', dot: '#10B981' };
