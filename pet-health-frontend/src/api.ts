...
export async function listSenUsers(token: string, options: { q?: string; limit?: number } = {}) {
  const params = new URLSearchParams();
  if (options.q?.trim()) params.set('q', options.q.trim());
  if (options.limit != null) params.set('limit', String(options.limit));
  const qs = params.toString();
  return requestJson<{
    data: Array<{ user_id: string; display_name?: string | null; email?: string | null }>;
...
export async function confirmListingDeposit(
  token: string,
  postId: string,
  payload: { acknowledge: boolean; senUserId?: string },
) {
...
export async function requestListingDispute(
  token: string,
  postId: string,
  payload: { message: string; photoUris: string[] },
) {
  const formData = new FormData();
  formData.append('message', payload.message);
  for (let i = 0; i < payload.photoUris.length; i += 1) {
    await appendImageFileToFormData(
      formData,
      'photos',
      payload.photoUris[i],
  ...
  return requestJson<{ data: PetFeedPost; report?: unknown }>(
...
export async function requestListingCancelDeposit(
  token: string,
  postId: string,
  payload: { reason: string; photoUris?: string[] },
) {
  const formData = new FormData();
  formData.append('reason', payload.reason);
  const photos = payload.photoUris || [];
