...
import { APP_LINKS, isMoitBadgeLive } from '../config';
import {
  LEGAL_COMPANY_PHONE,
  LEGAL_CONTACT_EMAIL,
...
function MoitBadgePlaceholder({ compact }: { compact?: boolean }) {
  ...
  const live = isMoitBadgeLive(url);

  if (live) {
    return (
      <Pressable
        testID="moit-badge-live"
  ...
  return (
    <View
      ...
      accessibilityLabel={t('legal.moitBadgeSlotA11y')}
      className={`items-center justify-center rounded-xl border border-dashed border-slate-300 bg-transparent ${
        compact ? 'px-3 py-3' : 'px-4 py-4'
      }`}
    >
      <Text className="text-center text-xs leading-4 text-slate-400">{t('legal.moitBadgeSlot')}</Text>
    </View>
  );
}
